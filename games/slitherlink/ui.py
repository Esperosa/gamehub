...
from PySide6.QtGui import QColor, QPainter, QPen, QFont, QBrush, QLinearGradient, QPainterPath
...
try:
    ...
    _engine_spec = importlib.util.spec_from_file_location(
        "slitherlink_engine", _this_dir / "engine.py"
    )
...
class SlitherlinkBoard(QWidget):
    ...
    def _draw_no_templates(self, painter: QPainter):
        ...
        painter.drawText(
            QRectF(left, cy + size + 20, board_size, 40), Qt.AlignCenter, "Žádné šablony"
        )
    ...
    def _draw_clues(self, painter: QPainter, left: float, top: float, cell: float):
        ...
        for r in range(puzzle.height):
            for c in range(puzzle.width):
                ...
                # Prefer solution-aware feedback when the generated puzzle carries solution data.
                # This prevents false "green" states where count matches but specific edges are wrong.
                if solution_h and solution_v:
                    expected = (
                        bool(solution_h[r][c]),  # top
                        bool(solution_v[r][c + 1]),  # right
                        bool(solution_h[r + 1][c]),  # bottom
                        bool(solution_v[r][c]),  # left
                    )
                    wrong = False
                    has_all_required_lines = True
                    for edge_state, should_be_line in zip(edges, expected):
                        if edge_state == 1 and not should_be_line:
                            wrong = True
                            break
                        if edge_state == -1 and should_be_line:
                            wrong = True
                            break
                        if should_be_line and edge_state != 1:
                            has_all_required_lines = False

                    if wrong:
                        color = COLOR_CLUE_ERROR
                    elif has_all_required_lines:
                        # Consider clue satisfied once all required solution lines are in place.
                        # Non-required edges may remain unknown without blocking green feedback.
                        color = COLOR_CLUE_SATISFIED
                    else:
                        color = COLOR_CLUE
                else:
                    # Fallback when no solution data is available.
                ...
                painter.drawText(QRectF(x, y, cell, cell), Qt.AlignCenter, str(clue))
    ...
    def _draw_overlay(self, painter: QPainter):
        ...
        painter.drawText(QRectF(0, cy - 80, self.width(), 60), Qt.AlignCenter, self._overlay_title)
        ...
        painter.drawText(
            QRectF(0, cy - 20, self.width(), 30), Qt.AlignCenter, self._overlay_subtitle
        )
    ...
    def _on_print(self):
        if self._loading_puzzle:
            QMessageBox.information(
                self, "Slitherlink", "Počkej na dokončení načítání aktuální hry."
            )
            return
        ...
        if not items:
            QMessageBox.warning(
                self, "Slitherlink", "Nepodařilo se vygenerovat žádnou úlohu pro tisk."
            )
            return
