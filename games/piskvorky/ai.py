...
def evaluate_position_fast(board: np.ndarray, n: int32, win_len: int32, player: int8) -> int32:
    """
...
@njit(cache=True, fastmath=True)
def evaluate_board_fast(board: np.ndarray, n: int32, win_len: int32, perspective: int8) -> int32:
...
@njit(cache=True, fastmath=True)
def score_move_fast(
    board: np.ndarray, move: int32, n: int32, win_len: int32, player: int8
) -> int32:
...
def order_moves_fast(
    board: np.ndarray, moves: np.ndarray, n: int32, win_len: int32, player: int8
) -> np.ndarray:
...
@njit(cache=True)
def check_immediate_win(board: np.ndarray, n: int32, win_len: int32, player: int8) -> int32:
...
def minimax_ab(
    ...
    if maximizing:
        ...
        for i in range(len(moves)):
            m = int(moves[i])
            board[m] = player
            score, _ = minimax_ab(board, n, win_len, depth - 1, alpha, beta, False, player, tt)
            board[m] = 0
        ...
        for i in range(len(moves)):
            m = int(moves[i])
            board[m] = -player
            score, _ = minimax_ab(board, n, win_len, depth - 1, alpha, beta, True, player, tt)
            board[m] = 0
...
def evaluate(state: GameState, perspective: int) -> int:
    ...
    return int(
        evaluate_board_fast(board, np.int32(state.n), np.int32(state.win_len), np.int8(perspective))
    )
...
def best_move_medium(state: GameState, bot: int) -> int:
    ...
    - Always takes wins
    - Sometimes misses blocks (20% miss rate)
    - Finds forks more often
    ...
    # Block immediate threats (80% - still makes mistakes, but less often)
    if random.random() < 0.8:
        block = find_winning_move(state, opp)
        if block is not None:
            return block

    # Forks (40% chance - medium should see tactical motifs more often)
    if random.random() < 0.4:
        forks = find_fork_moves(state, bot)
        if forks:
            return random.choice(forks)

    # Block forks (30% - still imperfect, but less naive)
    ...
    if random.random() < 0.6:
        board = np.array(state.board, dtype=np.int8)
        tt = {}
        _, move = minimax_ab(board, state.n, state.win_len, 2, -100000000, 100000000, True, bot, tt)
        if move >= 0:
            return move
...
def best_move_hard(state: GameState, bot: int, time_limit: float = 0.55) -> SearchResult:
    """
    HARD AI - Deep strategic search with transposition table.
    Target: ~10% stronger than previous baseline while staying responsive.
...
def _search_iterative(
    state: GameState, bot: int, max_depth: int, time_limit: float, tt: Dict
) -> SearchResult:
    ...
    for d in range(2, max_depth + 1):
        if time.perf_counter() - start > time_limit * 0.88:
            break

        score, move = minimax_ab(
            board.copy(), state.n, state.win_len, d, -100000000, 100000000, True, bot, tt
        )
