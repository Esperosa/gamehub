...
def create_game(
    ...
    return MastermindGame(secret=secret, num_colors=num_colors, max_attempts=max_attempts)
...
def filter_compatible_codes(
    codes: List[List[Color]], guess: List[Color], feedback: Feedback
) -> List[List[Color]]:
    """
    Filter codes that are compatible with the given guess and feedback.
    Used for AI/hint functionality.
    """
    return [code for code in codes if calculate_feedback(guess, code) == feedback]
...
def suggest_guess(game: MastermindGame) -> List[Color]:
    ...
    best_guess = None
    best_score: Tuple[int, int] = (10**9, 1)
