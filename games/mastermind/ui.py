...
class MastermindBoard(QWidget):
    ...
    def _draw_guess_history(self, painter: QPainter) -> None:
        ...
        # Responsive sizing based on both width and available vertical space.
        peg_size = min(
            board_w / (self.game.code_length + 2.2),
            board_h / (self.game.max_attempts + 3.6),
            150.0,
        )
        peg_size = max(30.0, peg_size)
        peg_gap = max(8.0, peg_size * 0.18)
        row_gap = max(6.0, peg_size * 0.12)
        row_height = peg_size + row_gap
        label_w = max(30.0, peg_size * 0.68)
        start_y = top + max(8.0, peg_size * 0.2)
        ...
        for i, guess in enumerate(self.game.guesses):
            ...
            # Draw attempt number
            painter.setPen(QPen(COLOR_MUTED))
            font = QFont("Segoe UI", max(10, int(peg_size * 0.28)))
            painter.setFont(font)
            painter.drawText(QRectF(left, y, label_w, peg_size), Qt.AlignCenter, f"{i+1}")
            
            # Draw guess pegs
            peg_x = left + label_w + peg_gap
            ...
            # Draw feedback
            fb_size = peg_size * 0.48
            fb_gap = max(3.0, fb_size * 0.25)
            fb_x = peg_x + self.game.code_length * (peg_size + peg_gap) + max(12.0, peg_size * 0.35)
            
            # Correct pegs (white with green checkmark) - was black
            ...
            # Wrong position pegs (black with red X) - was white
    ...
    def _draw_current_guess(self, painter: QPainter) -> None:
        ...
        # Position current guess below history and above picker.
        history_height = len(self.game.guesses) * (history_peg + row_gap) + max(44.0, peg_size * 0.72)
        ...
        max_y = self.height() - picker_reserve - peg_size
        y = min(y, max_y)
        ...
        painter.setFont(font)
        painter.drawText(QRectF(left, y - 30, board_w, 25), Qt.AlignCenter, 
    ...
    def _draw_color_picker(self, painter: QPainter) -> None:
        ...
        start_x = (self.width() - total_width) / 2
        y = self.height() - (peg_size + max(40.0, peg_size * 1.02))
        ...
        painter.drawText(QRectF(left, label_y, board_w, label_height), Qt.AlignCenter,
        ...
        for i, color_enum in enumerate(self.game.available_colors()):
            ...
            painter.setFont(font)
            painter.drawText(QRectF(x, y + peg_size + 4, peg_size, 18), 
    ...
    def _draw_info(self, painter: QPainter) -> None:
        ...
        if self.game.guesses and not self.game.is_over:
            ...
            painter.drawText(QRectF(10, self.height() - 25, self.width() - 20, 20), 
    ...
    def _draw_overlay(self, painter: QPainter) -> None:
        ...
        painter.setFont(font)
        painter.drawText(QRectF(0, center_y - 60, self.width(), 50),
                        Qt.AlignCenter, self._overlay_title)
        ...
        if self.game and self.game.is_lost:
            ...
            font = QFont("Segoe UI", 14)
            painter.setFont(font)
            painter.drawText(QRectF(0, center_y - 10, self.width(), 30),
                            Qt.AlignCenter, self._overlay_subtitle)
